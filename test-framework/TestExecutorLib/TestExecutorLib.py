...
# exceptions
class AgentDownException(Exception): pass
class DeprecatedException(Exception): pass
class StepException(Exception): pass
class BreakpointException(Exception): pass
class TestWaitException(Exception): pass
...
class TestSuitesManager(object):
    ...
    def testcases(self):
        ...
        for tsId, ts in self.tcs.items():
            if ts['is-testplan'] and ts['is-tp-started']: 
                sub_name = ts["name"]
                if "alias" in ts: 
                    if len(ts["alias"]):  sub_name = ts["alias"]
                subtp = sub_name
            ...
            for tc in ts['tc']:
                ...
                if self.__tp_name is not None:
                    ...
                    ts_name = ts["name"]
                    if "alias" in ts: 
                        if len(ts["alias"]): ts_name = ts["alias"]
                        
                    if subtp is not None:
                        testcase["parent"] = "%s\%s" % ( subtp, ts_name )
                    else:
    ...
    def initialize(self, path, testname, replayId, userId, projectId, stepByStep, 
        ...
        self.__filename = "%s_%s.%s" % ( testname, replayId, TEST_RESULT_VERDICT_EXT )
        self.__filename_report = "%s_%s.%s" % ( testname, replayId, TEST_RESULT_REPORT_EXT )
        self.__filename_design = "%s_%s.%s" % ( testname, replayId, TEST_RESULT_DESIGN_EXT )

        self.__filename_xml = "%s_%s.%s" % ( testname, replayId, TEST_RESULT_VERDICT_XML_EXT )
        self.__filename_report_xml = "%s_%s.%s" % ( testname, replayId, TEST_RESULT_REPORT_XML_EXT )
        self.__filename_design_xml = "%s_%s.%s" % ( testname, replayId, TEST_RESULT_DESIGN_XML_EXT )
        
        # new in v13.1
        self.__filename_basic_report = "%s_%s.%s" % ( testname, replayId, TEST_RESULT_BASIC_REPORT_EXT )
        ...
        try:
            ...
            for tsId, ts in self.tcs.items():
                ...
                for tc in ts['tc']:
                    ...
                    design = []
                    design.append( u"\t\t<user>%s</user>" % xmlescape(self.__username) )
                    design.append( u"\t\t<fileproject>%s</fileproject>" % xmlescape(self.__projectname) )
                    design.append( u"\t\t<filepath>%s</filepath>" % xmlescape(self.__testpath) )
                    design.append( u"\t\t<filename>%s</filename>" % xmlescape(self.__testname) )
                    design.append( u"\t\t<testproject>%s</testproject>" % xmlescape(ts["project"])  )
                    design.append( u"\t\t<testpath>%s</testpath>" % xmlescape(ts["path"])  )
                    
                    # new in v16
                    tsName = xmlescape(ts['name'])
                    if len(ts['alias']): tsName = xmlescape(ts['alias'])
                    # end of new
    ...
    def saveDesigns(self):
        ...
        try:
            ...
            designs.append( u'<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />' )
    ...
    def __initBasicReportTpl(self):
        ...
        """
        tpl_path = "%s/basic_report.tpl" % ( TestSettings.get( 'Paths', 'templates' ) )
        try:
            fd = open( tpl_path , "r")
            tpl_report = fd.read()
            fd.close()
    ...
    def __initReportHeaderTpl(self):
        ...
        Initiliaze the report template
        """
        tpl_path = "%s/report_header.tpl" % ( TestSettings.get( 'Paths', 'templates' ) )
        try:
            fd = open( tpl_path , "r")
            tpl_report = fd.read()
            fd.close()
            ...
            js_lib = ''
            try:
                fd = open( "%s/chart.bundle.min.%s.js" % (  TestSettings.get( 'Paths', 'templates' ),   CHART_JS_VERSION ) , "r")
                js_lib = fd.read()
                fd.close()
            except Exception as e:
    ...
    def getHeaderReport(self):
        ...
        """
        return  u"<div class='report_header'>Test report auto-generated by <a href='http://www.extensivetesting.org/'>Extensive Testing</a> - Copyright (c) 2010-2017 - Denis Machard</div>" 
    ...
    def prepareParameters(self, parameters):
        ...
        for i in parameters:
            if i['type'] in ['snapshot-image', 'local-file', 'local-image', 'dataset' ]:
                inputs_str.append( u'* %s (%s) = ...binary...' % (i['name'], i['type']) )
            else:
                if i['type'] in [ "shared" ]: 
                    val_tmp  = TestPropertiesLib.instance().readShared(shared=i['value'])
    ...
    def prepareStatistics(self, statistics, statisticsTu, statisticsTa, statisticsTs, statisticsTp, statisticsTg):
        ...
        total = (statistics['PASSED']+statistics['FAILED']+statistics['UNDEFINED']) 
        if total > 0:
        ...
        total_tu = (statisticsTu['PASSED']+statisticsTu['FAILED']+statisticsTu['UNDEFINED']) 
        if total_tu > 0:
        ...
        # prepare testabstract stats
        total_ta = (statisticsTa['PASSED']+statisticsTa['FAILED']+statisticsTa['UNDEFINED']) 
        if total_ta > 0:
            pass_test_ta = (statisticsTa['PASSED'] * 100 ) / total_ta
            fail_test_ta = (statisticsTa['FAILED'] * 100 ) / total_ta
            undef_test_ta =(statisticsTa['UNDEFINED'] * 100 ) / total_ta
        else:
            pass_test_ta = 0
            fail_test_ta = 0
            undef_test_ta = 0
        ...
        total_ts = (statisticsTs['PASSED']+statisticsTs['FAILED']+statisticsTs['UNDEFINED']) 
        if total_ts > 0:
        ...
        total_tp = (statisticsTp['PASSED']+statisticsTp['FAILED']+statisticsTp['UNDEFINED']) 
        if total_tp > 0:
        ...
        total_tg = (statisticsTg['PASSED']+statisticsTg['FAILED']+statisticsTg['UNDEFINED']) 
        if total_tg > 0:
        ...
        ret.append("<tr><td></td><td>PASS</td><td>FAIL</td><td>UNDEF</td><td>TOTAL</td></tr>")
        
        tmp_tc = [ "<tr><td>TESTCASE:</td><td>%s " % statistics['PASSED'] ]
        if pass_test > 0: tmp_tc.append("<small><font color='#008000'>(%s%%)</font></small>" % pass_test)
        tmp_tc.append( "</td><td>%s "  % statistics['FAILED'] )    
        if fail_test > 0: tmp_tc.append("<small><font color='#FF00000'>(%s%%)</font></small>" % fail_test)
        tmp_tc.append( "</td><td>%s "  % statistics['UNDEFINED'] )    
        if undef_test > 0: tmp_tc.append("<small>(%s%%)</small>" % undef_test)
        tmp_tc.append( "</td><td>%s</td></tr>" % total )
        ret.append( "".join(tmp_tc) )

        tmp_ta = [ "<tr><td>TESTABSTRACT:</td><td>%s " % statisticsTa['PASSED'] ]
        if pass_test_ta > 0: tmp_ta.append("<small><font color='#008000'>(%s%%)</font></small>" % pass_test_ta)
        tmp_ta.append( "</td><td>%s "  % statisticsTa['FAILED'] )    
        if fail_test_ta > 0: tmp_ta.append("<small><font color='#FF00000'>(%s%%)</font></small>" % fail_test_ta)
        tmp_ta.append( "</td><td>%s "  % statisticsTa['UNDEFINED'] )    
        if undef_test_ta > 0: tmp_ta.append("<small>(%s%%)</small>" % undef_test_ta)
        tmp_ta.append( "</td><td>%s</td></tr>" % total_ta )
        ret.append( "".join(tmp_ta) )

        tmp_tu = [ "<tr><td>TESTUNIT:</td><td>%s " % statisticsTu['PASSED'] ]
        if pass_test_tu > 0: tmp_tu.append("<small><font color='#008000'>(%s%%)</font></small>" % pass_test_tu)
        tmp_tu.append( "</td><td>%s "  % statisticsTu['FAILED'] )    
        if fail_test_tu > 0: tmp_tu.append("<small><font color='#FF00000'>(%s%%)</font></small>" % fail_test_tu)
        tmp_tu.append( "</td><td>%s "  % statisticsTu['UNDEFINED'] )    
        if undef_test_tu > 0: tmp_tu.append("<small>(%s%%)</small>" % undef_test_tu)
        tmp_tu.append( "</td><td>%s</td></tr>" % total_tu )
        ret.append( "".join(tmp_tu) )

        tmp_ts = [ "<tr><td>TESTSUITE:</td><td>%s " % statisticsTs['PASSED'] ]
        if pass_test_ts > 0: tmp_ts.append("<small><font color='#008000'>(%s%%)</font></small>" % pass_test_ts)
        tmp_ts.append( "</td><td>%s "  % statisticsTs['FAILED'] )    
        if fail_test_ts > 0: tmp_ts.append("<small><font color='#FF00000'>(%s%%)</font></small>" % fail_test_ts)
        tmp_ts.append( "</td><td>%s "  % statisticsTs['UNDEFINED'] )    
        if undef_test_ts > 0: tmp_ts.append("<small>(%s%%)</small>" % undef_test_ts)
        tmp_ts.append( "</td><td>%s</td></tr>" % total_ts )
        ret.append( "".join(tmp_ts) )

        tmp_tp = [ "<tr><td>TESTPLAN:</td><td>%s " % statisticsTp['PASSED'] ]
        if pass_test_tp > 0: tmp_tp.append("<small><font color='#008000'>(%s%%)</font></small>" % pass_test_tp)
        tmp_tp.append( "</td><td>%s "  % statisticsTp['FAILED'] )    
        if fail_test_tp > 0: tmp_tp.append("<small><font color='#FF00000'>(%s%%)</font></small>" % fail_test_tp)
        tmp_tp.append( "</td><td>%s "  % statisticsTp['UNDEFINED'] )    
        if undef_test_tp > 0: tmp_tp.append("<small>(%s%%)</small>" % undef_test_tp)
        tmp_tp.append( "</td><td>%s</td></tr>" % total_tp )
        ret.append( "".join(tmp_tp) )

        tmp_tg = [ "<tr><td>TESTGLOBAL:</td><td>%s " % statisticsTg['PASSED'] ]
        if pass_test_tg > 0: tmp_tg.append("<small><font color='#008000'>(%s%%)</font></small>" % pass_test_tg)
        tmp_tg.append( "</td><td>%s "  % statisticsTg['FAILED'] )    
        if fail_test_tg > 0: tmp_tg.append("<small><font color='#FF00000'>(%s%%)</font></small>" % fail_test_tg)
        tmp_tg.append( "</td><td>%s "  % statisticsTg['UNDEFINED'] )    
        if undef_test_tg > 0: tmp_tg.append("<small>(%s%%)</small>" % undef_test_tg)
    ...
    def computeStatistics(self):
        ...
        statsTa = {'PASSED': 0, 'FAILED': 0, 'UNDEFINED': 0 }
        ...
        for tsId, ts in self.tcs.items():
            ...
            isTestPlan += 1
            for tc in ts['tc']:
                if tc.get_final_verdict() == PASS: stats['PASSED'] += 1
                if tc.get_final_verdict() == FAIL: stats['FAILED'] += 1
                if tc.get_final_verdict() == UNDEFINED: stats['UNDEFINED'] += 1

                if ts['is-unit']:
                    if tc.get_final_verdict() == PASS: statsTu['PASSED'] += 1
                    if tc.get_final_verdict() == FAIL: statsTu['FAILED'] += 1
                    if tc.get_final_verdict() == UNDEFINED: statsTu['UNDEFINED'] += 1
                elif ts['is-abstract']:
                    if tc.get_final_verdict() == PASS: statsTa['PASSED'] += 1
                    if tc.get_final_verdict() == FAIL: statsTa['FAILED'] += 1
                    if tc.get_final_verdict() == UNDEFINED: statsTa['UNDEFINED'] += 1
                else: # is testsuite
                    isTs = True
                ...
                if finalResult in [ FAIL, UNDEFINED ] : continue
                finalResult = tc.get_final_verdict()
            
            if tsResult is None: tsResult = UNDEFINED
            if isTs :
                if tsResult == PASS: statsTs['PASSED'] += 1
                if tsResult == FAIL: statsTs['FAILED'] += 1
                if tsResult == UNDEFINED: statsTs['UNDEFINED'] += 1
                
            # test plan in test global
            if ts['is-testplan'] and ts['is-tp-started']:
                isTestGlobal = True
                if ts['result'] == PASS: statsTp['PASSED'] += 1
                if ts['result'] == FAIL: statsTp['FAILED'] += 1
                if ts['result'] == UNDEFINED: statsTp['UNDEFINED'] += 1

        if stats['UNDEFINED'] > 0 : finalResult = UNDEFINED
        if finalResult is None: finalResult = UNDEFINED
        
        # a test global is running ?
        if isTestGlobal:
            if finalResult == PASS: statsTg['PASSED'] += 1
            if finalResult == FAIL: statsTg['FAILED'] += 1
            if finalResult == UNDEFINED: statsTg['UNDEFINED'] += 1
        # a test plan is running ?
        else:
            if isTestPlan > 1:
                if finalResult == PASS: statsTp['PASSED'] += 1
                if finalResult == FAIL: statsTp['FAILED'] += 1
                if finalResult == UNDEFINED: statsTp['UNDEFINED'] += 1

        self.__final_result = finalResult
        
        return(stats, statsTa, statsTu, statsTs, statsTp, statsTg)
    ...
    def saveBasicReports(self, stoppedAt=0):
        ...
        try:
            ...
            stats, statsTa, statsTu, statsTs, statsTp, statsTg = self.computeStatistics()
            ...
            if self.__tp_name is not None:
                if self.__tp_global:
                    if self.__final_result == PASS: 
                        ...
                        reports.append( u'<input type="checkbox" id="c0" checked  class="isexpanded" />')
                        reports.append( u'<label for="c0"><font color="#008000">%s</font></label>' % (self.__tp_name) )
                    elif self.__final_result == FAIL: 
                        reports.append( u'<li data-type="failed">')
                        reports.append( u'<input type="checkbox" id="c0" checked  class="isexpanded" />')
                        reports.append( u'<label for="c0"><font color="#FF00000">%s</font></label>' % (self.__tp_name) )
                    else: 
                        reports.append( u'<li data-type="undefined">')
                        reports.append( u'<input type="checkbox" id="c0" checked  class="isexpanded" />')
                        reports.append( u'<label for="c0"><font color="#FFBB3D">%s</font></label>' % (self.__tp_name) )
                else:
                ...
                reports.append(  u" <span class=\"test_time\">(%s, %.3f seconds)</span >" % (self.__tp_startedat, self.__tp_duration) )
            ...
            for tsId, ts in self.tcs.items():
                ...
                tsName = ts['name']
                if len(ts['alias']): tsName = ts['alias']
                ...
                if ts['is-unit']:
                    ...
                    checked = "checked"
                    if self.__tp_global: checked = ""
                    
                    v = ts["result"]
                    if v == PASS: 
                        reports.append( u'<li data-type="passed">')
                        reports.append( u'<input type="checkbox" id="c%s" %s  class="isexpanded" />' % (index, checked) )
                        reports.append( u'<label for="c%s"><font color="#008000">%s</font></label>' % (index,tsName) )
                    elif v == FAIL: 
                        reports.append( u'<li data-type="failed">')
                        reports.append( u'<input type="checkbox" id="c%s" %s  class="isexpanded" />' % (index, checked) )
                ...
                for tc in ts['tc']:
                    ...
                    if v == PASS:
                        ...
                        reports.append( u'<input type="checkbox" id="c%st%s"  class="isexpanded" />' % (index, indexTc) )
                        reports.append( u'<label for="c%st%s"><font color="#008000">%s</font></label>' % (index,indexTc,n) )
                    
                        if len(tc.getLogs()):
                            reports.append( u'<input type="checkbox" id="info-c%st%s" class="isexpanded3" />' % (index,indexTc) )
                            reports.append( u'<label for="info-c%st%s"><span>[logs details]</span></label>' % (index,indexTc) )
                            
                    elif v == FAIL:
                        reports.append( u'<li data-type="fail">' )
                        reports.append( u'<input type="checkbox" id="c%st%s" class="isexpanded" />' % (index, indexTc) )
                        reports.append( u'<label for="c%st%s"><font color="#FF00000">%s</font></label>' % (index,indexTc,n) )
                        
                        if len(tc.getErrors()):
                            reports.append( u'<input type="checkbox" id="err-c%st%s" class="isexpanded2" />' % (index,indexTc) )
                            reports.append( u'<label for="err-c%st%s"><span>[errors details]</span></label>' % (index,indexTc) )
                        ...
                        reports.append( u'<input type="checkbox" id="c%st%s"  class="isexpanded" />' % (index, indexTc) )
                        reports.append( u'<label for="c%st%s"><font color="#FFBB3D">%s</font></label>' % (index,indexTc,n) )
                    ...
                    if len(tc.getErrors()):
                        ...
                        for e in tc.getErrors():
                            reports.append( u'<p>%s</p>' % xmlescape( filter_non_printable( e ) ) )
                    ...
                    for stp in tc.getSteps():
                        ...
                        sum = stp.getSummary()
                        if sv == PASS:
                            reports.append( u'<li data-type="pass"><font color="#008000">%s - %s</font></li>' % (s, sum) )
                        elif sv == FAIL:
                            reports.append( u'<li data-type="fail"><font color="#FF00000">%s - %s</font></li>' % (s, sum) )
                        else:
                            reports.append( u'<li data-type="undef"><font color="#FFBB3D">%s - %s</font></li>' % (s, sum) )
    ...
    def saveReports(self, stoppedAt=0):
        ...
        try:
            ...
            if self.__final_result == FAIL:
                reports.append(  u"<tr><td class=\"header\">Global Result</td><td><img alt=\"FAILED\" src=\"data:image/png;base64,%s\" /></td></tr>" % PNG_BASE64_FAILED)
            elif self.__final_result == UNDEFINED:
                reports.append(  u"<tr><td class=\"header\">Global Result</td><td><img alt=\"UNDEFINED\" src=\"data:image/png;base64,%s\" /></td></tr>" % PNG_BASE64_UNDEFINED)
            else:
                reports.append(  u"<tr><td class=\"header\">Global Result</td><td><img alt=\"PASSED\" src=\"data:image/png;base64,%s\" /></td></tr>" % PNG_BASE64_PASSED)
                
            reports.append(  u"<tr><td class=\"header\">Duration</td><td>%.3f seconds</td></tr>" %  self.scriptDuration)
            reports.append(  u"<tr><td class=\"header\">Statistics</td><td>%s</td></tr>" % self.prepareStatistics(stats, statsTu, statsTa, statsTs, statsTp, statsTg) )
            reports.append(  u"<tr><td class=\"header\">Terminated At</td><td>%s</td></tr>" % time.strftime( "%d/%m/%Y %H:%M:%S", time.localtime(stoppedAt) ) )
            for (k,v) in self.description:
                if isinstance(v, list):
                    reports.append(  u"<tr><td class=\"header\">%s</td><td>%s</td></tr>" % (k.title(), u'<br />'.join(v) ) )
                else:
                    reports.append(  u"<tr><td class=\"header\">%s</td><td>%s</td></tr>" % (k.title(),v) )
            reports.append(  u"</table>" )
            reports.append( u'</li><br />' )
            
            # new in v16
            if TestSettings.get( 'Tests_Framework', 'expand-test-report' ) == "1" :
                expanded = "checked"
            else:
                expanded = ""
            # end of new
            ...
            if self.__tp_name is not None:
                if self.__tp_global:
                    ...
                    reports.append( u'<input type="checkbox" id="c0" %s />' % expanded)
                    reports.append(  u"<label for=\"c0\"><h1>Test Global - %s</h1></label>" % self.__tp_name )
                else:
                    reports.append( u'<li data-type="testplan">' )
                    reports.append( u'<input type="checkbox" id="c0" %s />' % expanded)
                    reports.append(  u"<label for=\"c0\"><h1>Test Plan - %s</h1></label>" % self.__tp_name )
                    
                reports.append( u'<ul>' )
                
                reports.append(  u"<table id='description'>" )
                reports.append(  u"<tr><td class=\"header\">Summary</td><td>%s</td></tr>" %  self.__tp_datasummary)
                reports.append(  u"<tr><td class=\"header\">Started At</td><td>%s</td></tr>" % self.__tp_startedat)
                reports.append(  u"<tr><td class=\"header\">Duration</td><td>%.3f seconds</td></tr>" %  self.__tp_duration)
                if len(self.__tp_datainputs):
                    reports.append(  u"<tr><td class=\"header\">Data Inputs</td><td>%s</td></tr>" % u"<br />".join(self.prepareParameters(self.__tp_datainputs)) )
                if len(self.__tp_sutinputs):
                    reports.append(  u"<tr><td class=\"header\">SUT Inputs</td><td>%s</td></tr>" % u"<br />".join(self.prepareParameters(self.__tp_sutinputs)) )
                if len(self.__tp_dataoutputs):
                    reports.append(  u"<tr><td class=\"header\">Data Outputs</td><td>%s</td></tr>" % u"<br />".join(self.prepareParameters(self.__tp_dataoutputs)) )
                if len(self.__tp_sutoutputs):
                    reports.append(  u"<tr><td class=\"header\">SUT Outputs</td><td>%s</td></tr>" % u"<br />".join(self.prepareParameters(self.__tp_sutoutputs)) )
                reports.append(  u"</table>" )
            ...
            for tsId, ts in self.tcs.items():
                ...
                if not int(ts['is-enabled']) and not ts['is-testplan']:
                    if ts['is-unit']:
                        ...
                        reports.append(  u"<h1 class=\"test_disabled\"><i>Test Unit - %s [disabled]</i></h1>" % tsName)
                    elif ts['is-abstract']:
                        reports.append( u'<li data-type="testdisabled">' )
                        reports.append(  u"<h1 class=\"test_disabled\"><i>Test Abstract - %s [disabled]</i></h1>" % tsName)
                    elif ts['is-tp-from-tg']:
                        reports.append( u'<li data-type="testdisabled">' )
                        reports.append(  u"<h1 class=\"test_disabled\"><i>Test Plan - %s [disabled]</i></h1>" % tsName)
                    else:
                        reports.append( u'<li data-type="testdisabled">' )
                        reports.append(  u"<h1 class=\"test_disabled\"><i>Test Suite - %s [disabled]</i></h1>" % tsName )
                    continue
                # end of new
                ...
                if not ts['is-executed']:
                    ...
                    if ts['is-unit']:
                        ...
                        reports.append(  u"<h1 class=\"test_disabled\"><i>Test Unit - %s [not executed]</i></h1>" % tsName)
                    elif ts['is-abstract']:
                        reports.append( u'<li data-type="testnotexecuted">' )
                        reports.append(  u"<h1 class=\"test_disabled\"><i>Test Abstract - %s [not executed]</i></h1>" % tsName)
                    elif ts['is-tp-from-tg']:
                        reports.append( u'<li data-type="testnotexecuted">' )
                        reports.append(  u"<h1 class=\"test_disabled\"><i>Test Plan - %s [not executed]</i></h1>" % tsName)
                    else:
                        reports.append( u'<li data-type="testnotexecuted">' )
                        reports.append(  u"<h1 class=\"test_disabled\"><i>Test Suite - %s [not executed]</i></h1>" % tsName )
                    # end of new
                    continue
                ...
                if ts['is-unit']:
                    ...
                    reports.append( u'<input type="checkbox" id="c%s" %s />' % (index,expanded) )
                    reports.append(  u"<label for=\"c%s\"><h1>Test Unit - %s</h1></label>" % (index,tsName) )

                    reports.append( u'<ul>' )
                    reports.append(  u"<table id='description'>" )
                    reports.append(  u"<tr><td class=\"header\">Summary</td><td>%s</td></tr>" %  ts['data-summary'] )
                    reports.append(  u"<tr><td class=\"header\">Started At</td><td>%s</td></tr>" %  ts['started-at'])
                    reports.append(  u"<tr><td class=\"header\">Duration</td><td>%.3f seconds</td></tr>" %  ts['duration'])
                    if len(ts['data-inputs']):
                        reports.append(  u"<tr><td class=\"header\">Data Inputs</td><td>%s</td></tr>" % u"<br />".join(self.prepareParameters(ts['data-inputs'])) )
                    if len(ts['sut-inputs']):
                        reports.append(  u"<tr><td class=\"header\">SUT Inputs</td><td>%s</td></tr>" % u"<br />".join(self.prepareParameters(ts['sut-inputs'])) )
                    if len(ts['data-outputs']):
                        reports.append(  u"<tr><td class=\"header\">Data Outputs</td><td>%s</td></tr>" % u"<br />".join(self.prepareParameters(ts['data-outputs'])) )
                    if len(ts['sut-outputs']):
                        reports.append(  u"<tr><td class=\"header\">SUT Outputs</td><td>%s</td></tr>" % u"<br />".join(self.prepareParameters(ts['sut-outputs'])) )
                    reports.append(  u"</table>" )
                    
                elif ts['is-abstract']:
                    reports.append( u'<li data-type="testabstract">' )
                    reports.append( u'<input type="checkbox" id="c%s" %s />' % (index, expanded) )
                    reports.append(  u"<label for=\"c%s\"><h1>Test Abstract - %s</h1></label>" % (index,tsName))
                    ...
                    reports.append( u'<li data-type="testplan">' )
                    reports.append( u'<input type="checkbox" id="c%s" %s />' % (index, expanded) )
                    reports.append(  u"<label for=\"c%s\"><h1>Test Plan - %s</h1></label>" % (index,tsName) )
                    ...
                    reports.append( u'<input type="checkbox" id="c%s" %s />' % (index, expanded) )
                    reports.append(  u"<label for=\"c%s\"><h1>Test Suite: %s</h1></label>" % (index,tsName) )
                ...
                for tc in ts['tc']:
                    # i += 1
                    reports.append( u"<li class=\"step-section\">%s</li>" % tc.getStepsManager().getReport() )
    ...
    def saveToCsv(self):
        ...
        try:
            ...
            if self.__tp_name is not None:
                ...
                if self.__tp_global:
                    result.append( self.csv_separator.join( [ TG, tpname , '', '', self.__tp_verdict ] )  )
                else:
                    result.append( self.csv_separator.join( [ TP, tpname , '', '', self.__tp_verdict ] )  )
            ...
            for tsId, ts in self.tcs.items():
                ...
                if ts['is-testplan'] and ts['is-tp-started']:
                    ...
                    if 'alias' in ts: 
                        if len(ts['alias']): ts_name = ts['alias']
                    if len(tpname): tsname = "%s\%s" % (tpname, ts_name)
                    else: tsname = ts_name
                    
                    result.append( self.csv_separator.join( [ TP, tsname , '', '',  ts['result'] ] )  )
                    subtp = ts_name
                    continue
                if ts['is-testplan'] and not ts['is-tp-started']:
                    subtp = None
                ...
                if subtp is None:
                    ...
                    if 'alias' in ts: 
                        if len(ts['alias']): ts_name = ts['alias']
                    if len(tpname): tsname = "%s\%s\%s" % (tpname, subtp, ts_name)
                    else: tsname = ts_name
                
                if ts['is-unit']:
                    result.append( self.csv_separator.join( [ TU, tsname , '', '', ts['result'] ] )  )
                elif ts['is-abstract']:
                    result.append( self.csv_separator.join( [ TA, tsname , '', '',  ts['result'] ] )  )
                else:
                    result.append( self.csv_separator.join( [ TS, tsname , '', '',  ts['result'] ] )  )
                    
                # iterate each testcases
                for tc in ts['tc']:
                    result.append( self.csv_separator.join( [ TC, tsname, tc.getTestname() , '' , tc.get_final_verdict() ] )  )

                    # iterate each steps
                    for stp in  tc.getSteps():
                        result.append( self.csv_separator.join( [ STP,  tsname, tc.getTestname(), stp.getStep(), stp.getVerdict() ] )  )
        except Exception as e:
    ...
    def saveVerdictToXml(self):
        ...
        try:
            ...
            for tsId, ts in self.tcs.items():
                ...
                status = "not-executed"
                if ts['is-executed']: status = "executed"
                if not int(ts['is-enabled']) and not ts['is-testplan']: status = "disabled"
                ...
                verdicts.append( u"\t<test id=\"%s\" name=\"%s\" result=\"%s\" max=\"%s\" status=\"%s\" project=\"%s\" path=\"%s\" original=\"%s\" extension=\"%s\">" % (
                                                                                        i,
                                                                                        xmlescape(ts_name),
                                                                                        ts['result'], 
                                                                                        len(ts['tc']),
                                                                                        status,
                                                                                        xmlescape(ts['project']),
                                                                                        xmlescape(ts['path']),
                                                                                        xmlescape(ts['name']),
                                                                                        ext
                                                                                      )
                ...
                for tc in ts['tc']:
                    ...
                    verdicts.append( u"\t\t<testcase id=\"%s\" name=\"%s\" result=\"%s\" max=\"%s\" %s" % (
                                                                                        j,
                                                                                        xmlescape(tc.getTestname()),
                    ...
                    for stp in  tc.getSteps():
                        h += 1
                        verdicts.append( u"\t\t\t<step id=\"%s\" name=\"%s\" result=\"%s\" actual=\"%s\" />" % (
                                                                                        h, 
                                                                                        xmlescape(stp.getStep()),
                                                                                        stp.getVerdict(),
                                                                                        xmlescape(stp.getActual())
                                                                                      )
                                                                                    )
                    if len(tc.getSteps()):
    ...
    def newStartTpInTg(self, name, nameAlias='', summary='', startedAt='', testPath='/', testProject=''):
        """
        Add a start testplan in a testglobal
        """
        self.__sub_tp = self.newTs(name=name, isUnit=False, isEnabled=False, isTestPlan=True, 
                       startTestPlan=True, isExecuted=True, nameAlias=nameAlias, 
                       summary=summary, startedAt=startedAt, testPath=testPath, testProject=testProject)
        
    def newTs(self, name, isUnit=False, isAbstract=False, isEnabled=False, isTestPlan=False,
                    startTestPlan=False, isExecuted=False, nameAlias='',
                    dataInputs=[], sutInputs=[], summary='', startedAt='', dataOutputs=[], sutOutputs=[],
                    isTpFromTg=False, testPath='/', testProject='', dataDescriptions=[]):
        """
        Add a new testsuite
        """
    ...
    def newTu(self, name, isEnabled=False, nameAlias='', dataInputs=[], sutInputs=[], 
                    summary='', startedAt='', dataOutputs=[], sutOutputs=[], testPath='/', testProject='',
                    dataDescriptions=[]):
        """
        Add a new testunit
        """
        self.newTs(name=name, isUnit=True, isEnabled=isEnabled, nameAlias=nameAlias, dataInputs=dataInputs, 
                    sutInputs=sutInputs, summary=summary, startedAt=startedAt, dataOutputs=dataOutputs, 
                    sutOutputs=sutOutputs, testPath=testPath, testProject=testProject, dataDescriptions=dataDescriptions)

    def newTa(self, name, isEnabled=False, nameAlias='', dataInputs=[], sutInputs=[], 
                    summary='', startedAt='', dataOutputs=[], sutOutputs=[], testPath='/', testProject='',
                    dataDescriptions=[]):
        """
        Add a new testabtract
        """
        self.newTs(name=name, isAbstract=True, isEnabled=isEnabled, nameAlias=nameAlias, dataInputs=dataInputs, 
                    sutInputs=sutInputs, summary=summary, startedAt=startedAt, dataOutputs=dataOutputs, 
                    sutOutputs=sutOutputs, testPath=testPath, testProject=testProject, dataDescriptions=dataDescriptions)
    ...
    def addOutputs(self, dataOutputs=[], sutOutputs=[]):
        """
        """
        self.tcs[self.id__]['data-outputs'] = dataOutputs
        self.tcs[self.id__]['sut-outputs'] = sutOutputs
...
class TestCasesManager(object):
    ...
    def endingAll(self):
        ...
        for (tc, tcArgs) in self.__tcs:
            if tc.hasSharedAdapters(): tc.endingDelayed(**tcArgs)
...
class Trace(object):
    ...
    def info(self, txt, bold = False, italic=False, multiline=False, raw=False) :
        ...
        @type raw: boolean
        """ 
        self.tcparent.info( txt=txt, bold = bold, italic=italic, multiline=multiline, raw=raw )
    @doc_public                    
    def error(self, txt, bold = False, italic=False, multiline=False, raw=False):
        ...
        @type raw: boolean
        """
        self.tcparent.error( txt=txt, bold = bold, italic=italic, multiline=multiline, raw=raw )
    @doc_public                       
    def warning(self, txt, bold = False, italic=False, multiline=False, raw=False):
        ...
        @type raw: boolean
        """
        self.tcparent.warning( txt=txt, bold = bold, italic=italic, multiline=multiline, raw=raw )
...
class Interact(object):
    ...
    def interact(self, ask, timeout=30.0, default=None, cache=''):
        ...
        @rtype: string
        """
        rsp = self.tcparent.interact( ask=ask, timeout=timeout, default=default)
...
class BreakPoint(object):
    ...
    def __init__(self, parent):
        ...
        if not isinstance(parent, TestCase):
            raise BreakpointException("ERR_BKP_004: testcase expected (%s)" % type(parent) )
            
        if getTsMgr().getBreakpoint():
            rsp = self.tcparent.breakpoint(testId=getTsMgr().getTestId())
            if rsp is None:
                raise BreakpointException("ERR_BKP_003: System error on breakpoint action")
            else:
                if rsp == 'cancel':
                    raise BreakpointException("ERR_BKP_001: Breakpoint cancelled")
                elif rsp == 'continue':
                    pass
                else:
                    raise BreakpointException("ERR_BKP_002: System breakpoint - unknown response %s" % rsp )
...
class Time(object):
    ...
    @doc_public    
    def sleepUntil(self, dt="1970-01-01 00:00:00", fmt="%Y-%m-%d %H:%M:%S", localshift=True, delta=0):
        """
        ...
        @type delta: integer
        """
        self.tcparent.sleepUntil(dt=dt, fmt=fmt,localshift=localshift, delta=delta)
    @doc_public    
    ...
    @doc_public    
    def waitUntil(self, dt="1970-01-01 00:00:00", fmt="%Y-%m-%d %H:%M:%S", delta=0):
        """
        ...
        """
        try:
            self.tcparent.waitUntil(dt=dt.strip(), fmt=fmt.strip(), delta=delta)
        except ValueError as e:
            raise TimeException('ERR_TIME_001: bad date/time format provided - %s' % e)
    @doc_public        
    def utc(self, fmt="%Y-%m-%d %H:%M:%S", timeshift=False):
        """
        Return UTC time
        ...
        global _TimeShift
        if timeshift:
            shift = localTimeShift()
            setTimeShift(shift=shift)

            local_dt = datetime.datetime.fromtimestamp(time.mktime(time.gmtime()))
            shift_dt = datetime.timedelta(hours=0,minutes=0, seconds=int(_TimeShift))
            self.tcparent.warning("shift: %s" % shift_dt)
            new_time = local_dt + shift_dt
            return new_time.strftime(fmt)
        else:    
            return time.strftime(fmt, time.gmtime())
    @doc_public
    def local(self, fmt="%Y-%m-%d %H:%M:%S", timeshift=False):
        """
        Return local time
        ...
        if timeshift:
            ...
            local_dt = datetime.datetime.fromtimestamp(time.mktime(time.localtime()))
            shift_dt = datetime.timedelta(hours=0,minutes=0, seconds=int(_TimeShift))
            self.tcparent.warning("shift: %s" % shift_dt)
            new_time = local_dt + shift_dt
            return new_time.strftime(fmt)
...
class Step(object):
    ...
    def getDesignXml(self):
        ...
        ret.append( u"\t\t<summary><![CDATA[%s]]></summary>" % self.getSummary() )
        ret.append( u"\t\t<action><![CDATA[%s]]></action>" % self.getAction() )
        ret.append( u"\t\t<expected><![CDATA[%s]]></expected>" % self.getExpected() )
        ret.append( u"\t</step>" )
        return ret
    ...
    def getActual(self):
        ...
        try:
            if self.actual_thumbnail is not None:
                ...
                self.actual_.append( u"<img src=\"data:image/png;base64,%s\" alt=\"thumbnail\" >" % imgB64)
        except Exception as e:
            pass
    ...
    def getAction(self):
        ...
        try:
            if self.action_thumbnail is not None:
                ...
                ret = u"%s<br /><img src=\"data:image/png;base64,%s\" alt=\"thumbnail\" >" % (self.action_, imgB64)
        except Exception as e:
            pass
    ...
    def start(self):
        ...
        if not getTsMgr().getStepByStep():
            ...
            rsp = self.tcparent.pause(testId=getTsMgr().getTestId(), stepId=self.id_, stepSummary=stpSummary)
            if rsp is None:
                raise StepException("ERR_STP_004: System error on pause action for step %s" % self.id_ )
            else:
                if rsp == 'cancel':
                    ...
                    raise StepException("ERR_STP_003: System pause: unknown response %s for step %s" % (rsp,self.id_) )
        ...
        if runStep:
            ...
            if self.action_thumbnail is not  None:
                tpl_layer.addKey( name='action-thumbnail', data=self.action_thumbnail )
    ...
    def saveStep(self):
        ...
        Save step in file
        """
        step = { 'testcase': self.tcparent.getTestname(), 'id': self.getId(), 'result': self.getVerdict(), 
                'summary': self.getSummary(),  'expected': self.getExpected(), 'actual': self.getActual(), 
    ...
    def setFailed (self, actual, thumbnail=None, chart=None):
        ...
        if not ( isinstance(actual, str) or isinstance(actual, unicode) ):
            raise StepException("ERR_STP_007: bad type provided on actual step (%s)" % type(actual) )
        # end of new
        ...
        shortMsg = actual
        rawTxt = "Step %s [%s]\n\n%sExpected:\t%s" % (self.id_, self.verdict, TAB, 
                                                        self.expected_.decode('utf8'))
        if self.action_ is not None:
            rawTxt = "Step %s [%s]\n\n%sAction:\t%s\n\n%sExpected:\t%s" % ( self.id_, self.verdict, TAB,  self.action_.decode('utf8'), 
                                                                            TAB, self.expected_.decode('utf8'))       
        if actual is not  None:
            rawTxt += "\n\n%sActual:\t\n%s" % (  TAB, self.actual_raw )
        ...
        if thumbnail is not  None:
            tpl_layer.addKey( name='actual-thumbnail', data=self.actual_thumbnail )
        tpl_layer.addKey( name='result', data=self.verdict )
        tpl.addLayer(tpl_layer)
        ...
        continueRun = TestSettings.getInt( 'Tests_Framework', 'continue-on-step-error' )
        if not continueRun:
...
class StepManager(object):
    ...
    def getDesign(self, designId, testName='', testPath='', userName='', projectName=''):
        """
        Return design
        """
        ret = []
        ...
        design = self.parent.getDesignTemplate() % ( 
                                                        designId, 
                                                        userName, projectName, testPath, testName,
    ...
    def getDesignXml(self):
        ...
        """
        ret = [ ]
        ret.append( u'<testcase>%s</testcase>' % xmlescape(self.parent.getTestname()) )
        if self.parent.getRequirement():
            ret.append( u'<requirement><![CDATA[%s]]></requirement>' % self.parent.getRequirement() )
        else:
            ret.append( u'<requirement />')
        if self.parent.getPurpose():
            ret.append( u'<purpose><![CDATA[%s]]></purpose>' % self.parent.getPurpose() )
        else:
    ...
    def getReport(self, isUnit=False, isAbstract=False):
        ...
        for stp in self.getSteps():
            ...
            if v == FAIL or v == UNDEFINED:
                actuals.append( u'<font color="#FF0000">%s. %s - %s</font>' % (stp.getId(), v, stp.getActual() ) )
            else:
                actuals.append( u'<font color="#008000">%s. %s - %s</font>' % (stp.getId(), v, stp.getActual() ) )
    ...
    def getReportXml(self):
        ...
        ret.append ( u'\t<testcase><![CDATA[%s]]></testcase>' % self.parent.getTestname() )
        ret.append ( u'\t<verdict><![CDATA[%s]]></verdict>' % self.parent.get_final_verdict() )
        ret.append ( u'\t<purpose><![CDATA[%s]]></purpose>' % self.parent.getPurpose() )
        ...
        for stp in self.getSteps():
            ...
            actions.append( u"%s. %s" % (stp.getId(), stp.getAction() ) )
            expected.append( u"%s. %s" % (stp.getId(), stp.getExpected() ) )
            actuals.append( u'%s. %s - %s' % (stp.getId(), stp.getVerdict(), stp.getActual() ) )
        ret.append( u'\t<steps-summaries><![CDATA[%s]]></steps-summaries>' % '\n'.join(summaries)  )
        ret.append( u'\t<steps-actions><![CDATA[%s]]></steps-actions>' % '\n'.join(actions)  )
        ret.append( u'\t<steps-expected><![CDATA[%s]]></steps-expected>' % '\n'.join(expected)  )
        ret.append( u'\t<steps-actuals><![CDATA[%s]]></steps-actuals>' % '\n'.join(actuals)  )
        
        return ret
    def getPreviousStep(self):
        """
        """
        if len(self.steps_) <= 1: return None
...
def wait(timeout):
    ...
    try:
        ...
        raise TestWaitException("ERR_TE_002: wait initialization failed, wrong type: %s" % str(timeout) )
    TLX.instance().log_testsuite_info(message='waiting for %s sec ...' % str(timeout), component=TS, fromlevel=LEVEL_TE, tolevel=LEVEL_USER)
    time.sleep(timeout)
...
_TimeShift = 0
def setTimeShift(shift):
    """
    Update the time shift
    """
    global _TimeShift
    _TimeShift = int(shift)

def localTimeShift():
    """
    """
    f = open("%s/timeshift" % TestSettings.get('Paths', 'templates'))
    timeshift = f.read()
    f.close()
    return timeshift.strip()
...
class TestCase(object):
    ...
    def getDataStoragePath(self):
        ...
        return "%s/" % os.path.normpath(tmp_path)
    ...
    def appendFile(self, destname, data):
        ...
        try:
            ...
            self.error( "unable to append testcase in data storage: %s" % str(e) )
    ...
    def saveData(self, data):
        """
        Save data in the temporary storage. The temp storage is share between each testcases.
    ...
    def loadData(self):
        """
        Load data from the temporary storage. The temp storage is share between each testcases.
    ...
    @doc_public
    def addStep (self, expected, description, summary, enabled=True, thumbnail=None):
        """
        ...
        @rtype: Step
        """ 
        return self.__stepsManager.addStep(expected = expected, action = description, 
                                            summary=summary, enabled=enabled, thumbnail=thumbnail)
    ...
    def registerComponent (self, component, shared=False):
        ...
        if shared:
            if not int( TestSettings.get('Tests_Framework', 'dispatch-events-current-tc') ): self.__hasSharedAdps = True
            getAdpsMgr().addAdp(name=component.getAdapterId(), value=component)
        
        # register all adapters and libraries
        getAdpsMgrALL().addAdp(name=component.getAdapterId(), value=component)
    ...
    def execute(self, **kwargs):
        ...
        if not _ExecuteTest:
            ...
            try:
                ...
                # new in v12.1
                if not int( TestSettings.get('Tests_Framework', 'dispatch-events-current-tc') ):
                    tcMgr = getTcMgr()
                ...
                if callable(body):
                    ...
                    # except ContinueTestException as e:
                        # aborted =  'Abort reason: %s' % e
                        # test_continued = 'Test stopped: %s' % e
                    # except TerminateTestException as e:
                        # aborted =  'Abort reason: %s' % e
                        # test_terminated = 'Test stopped: %s' % e
                        ...
                        self.error( e, raw=True ) 
                        self.setFailed(internal=True)
                    except TestValidatorsLib.TestValidatorsException as e:
                        aborted=True
                        self.error( e ) 
                        self.setFailed(internal=True)
                        ...
                        self.setFailed(internal=True)
                    except TestLibraryLib.ValueException as e:
                        aborted=True
                        self.error( 'ERR_LIB_100: %s' % e, raw=True) 
                        self.setFailed(internal=True)
                    except TestAdapterLib.AdapterException as e:
                        aborted=True
                        self.error( 'ERR_ADP_200: %s' % e, raw=True ) 
                        self.setFailed(internal=True)
                    except TestLibraryLib.LibraryException as e:
                        aborted=True
                        self.error( 'ERR_LIB_200: %s' % e, raw=True) 
                        self.setFailed(internal=True)
                    except Exception as e:
                        aborted=True
                    ...
                    if int( TestSettings.get('Tests_Framework', 'dispatch-events-current-tc') ):
                        ...
                        try:
                            ...
                            # self.cleanup(**kwargs) # cleanup done in the test
                        except Exception as e:
                            self.setFailed(internal=True, internalErr=str(e))
                            TLX.instance().log_testcase_error(str(e), TC, self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, 
                                                                testInfo=self.__testInfo)
                        self.__cleanup() # internal cleanup
                        TLX.instance().log_testcase_info('END', TC, self.__id, color_=TLX.instance().INFO_TS, fromlevel=LEVEL_TE, 
                                                        tolevel=LEVEL_USER,  testInfo=self.__testInfo, flagEnd=True, flagBegin=False)
                    else:
                        if not self.__hasSharedAdps:
                            TLX.instance().log_testcase_internal(message='Cleaning',component=TC, tcid = self.__id, bold=False, italic=True, fromlevel=LEVEL_TE, 
                            ...
                            try:
                                ...
                                TLX.instance().log_testcase_error(str(e), TC, self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, 
                                                                    testInfo=self.__testInfo)
                            self.__cleanup() # internal cleanup
                            TLX.instance().log_testcase_info('END', TC, self.__id, color_=TLX.instance().INFO_TS, fromlevel=LEVEL_TE, tolevel=                                  LEVEL_USER, testInfo=self.__testInfo, flagEnd=True, flagBegin=False)
                else:
                ...
                if int( TestSettings.get('Tests_Framework', 'dispatch-events-current-tc') ):
                    ...
                    TLX.instance().log_testcase_stopped(id_ = self.__id, result = self.get_final_verdict(), duration=duration, 
                                                        prjId=tsMgr.getProjectID(), fromlevel=LEVEL_TE, tolevel=LEVEL_USER, 
                                                        testInfo=self.__testInfo)
                ...
                stop_time = time.time()
                duration = stop_time - self.start_time
                TLX.instance().log_testcase_stopped(id_ = self.__id, result = self.get_final_verdict(), 
                                                    duration=duration, prjId=tsMgr.getProjectID(), fromlevel=LEVEL_TE, 
                                                    tolevel=LEVEL_USER, testInfo=self.__testInfo)

            if stopped: raise ForceStopException(stopped)
            # new in v17.1
            # if test_continued: 
                # raise ForceContinueTestException(test_continued)
            # if test_terminated: 
                # raise TerminateTestException(test_terminated)
            # end of new
        ...
        for adpId, adp in self.__cpts.items():
            if not adp.isShared():
                try:
                    ...
                    TLX.instance().log_testcase_error( "timer reset: %s" % str(e), TC, self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo)
                try:
                    adp.onReset()
                except Exception as e:
                    TLX.instance().log_testcase_error(str(e), TC, self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo)
                adp.stop()
                adp.join()
    ...
    def endingDelayed(self, **kwargs):
        ...
        tsMgr = getTsMgr()
        try:
            TLX.instance().log_testcase_internal(message='Cleaning',component=TC, tcid = self.__id, bold=False, italic=True, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo)
            try:
                self.cleanup(aborted, **kwargs) # cleanup done in the test
            except Exception as e:
                self.setFailed(internal=True)
                TLX.instance().log_testcase_error(str(e), TC, self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo)
            TLX.instance().log_testcase_info('END', TC, self.__id, color_=TLX.instance().INFO_TS, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, 
                                            testInfo=self.__testInfo, flagEnd=True, flagBegin=False)
            
            stop_time = time.time()
            duration = stop_time - self.start_time
            TLX.instance().log_testcase_stopped(id_ = self.__id, result = self.get_final_verdict(), duration=duration, prjId=tsMgr.getProjectID(), fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo)
    ...
    def wait(self, timeout):
        ...
        try:
            ...
            raise TestWaitException("ERR_TE_003: wait initialization failed, wrong type: %s" % str(timeout) )
        if timeout < 0: raise TestWaitException("ERR_TE_003: wait can be negative: %s" % str(timeout) )
        
        self.__logs.append( 'waiting for %s sec ...' % str(timeout) )
        TLX.instance().log_testcase_info(message='waiting for %s sec ...' % str(timeout), component=TC,
                tcid=self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo )
        time.sleep(timeout)
    ...
    def sleep(self, timeout, localshift=True):
        ...
        try:
            ...
            raise TestWaitException("ERR_TE_003: sleep initialization failed, wrong type: %s" % str(timeout) )
        if timeout < 0: raise TestWaitException("ERR_TE_003: sleep can be negative: %s" % str(timeout) )
        
        TLX.instance().log_testcase_info(message='sleeping for %s sec ...' % str(timeout), component=TC,
                tcid=self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo )
        
        timeref = time.time()
        if localshift:
            previousShift = int(localTimeShift())
        else:
            previousShift = 0
        while True:
            time.sleep(0.1)
            if localshift: 
                shift = localTimeShift()
                if previousShift != int(shift):
                    TLX.instance().log_testcase_info(message='local shifting system time of %s sec ...' % str(shift), component=TC,
                            tcid=self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo )
                    previousShift = int(shift)
                    
                    setTimeShift(shift=shift)
            else:
                if previousShift != int(_TimeShift):
                    TLX.instance().log_testcase_info(message='shifting system time of %s sec ...' % str(_TimeShift), component=TC,
                            tcid=self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo )
                    previousShift = int(_TimeShift)
            if ( (time.time() + _TimeShift) - timeref ) >= timeout:
                break
    ...
    def sleepUntil(self, dt="1970-01-01 00:00:00", fmt="%Y-%m-%d %H:%M:%S", localshift=True, delta=0):
        ...
        if timeout < 0: raise TestWaitException("ERR_TE_003: sleep until can be negative: %s" % str(timeout) )
        
        TLX.instance().log_testcase_info(message='sleeping until %s ...' % dt, component=TC,
                tcid=self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo )
    ...
    def waitUntil(self, dt="1970-01-01 00:00:00", fmt="%Y-%m-%d %H:%M:%S", delta=0):
        ...
        if timeout < 0: raise TestWaitException("ERR_TE_003: wait until can be negative: %s" % str(timeout) )
        
        TLX.instance().log_testcase_info(message='waiting until %s ...' % dt, component=TC,
                tcid=self.__id, fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testInfo )
        time.sleep(timeout)
    ...
    def info (self, txt, bold = False, italic=False, multiline=False, raw=False) :
        ...
        if not isinstance(bold, bool): raise Exception("tc>info: bad value for the argument: bold=%s (%s)" % (bold, type(bold)) )
        if not isinstance(italic, bool): raise Exception("tc>info: bad value for the argument: italic=%s (%s)" % (italic, type(italic)) )
        if not isinstance(multiline, bool): raise Exception("tc>info: bad value for the argument: multiline=%s (%s)" % (multiline, type(multiline)) )
        if not isinstance(raw, bool): raise Exception("tc>info: bad value for the argument: raw=%s (%s)" % (raw, type(raw)) )
    ...
    def error (self, txt, bold = False, italic=False, multiline=False, raw=False):
        ...
        if not isinstance(bold, bool): raise Exception("tc>error: bad value for the argument: bold=%s (%s)" % (bold, type(bold)) )
        if not isinstance(italic, bool): raise Exception("tc>error: bad value for the argument: italic=%s (%s)" % (italic, type(italic)) )
        if not isinstance(multiline, bool): raise Exception("tc>error: bad value for the argument: multiline=%s (%s)" % (multiline, type(multiline)) )
        if not isinstance(raw, bool): raise Exception("tc>error: bad value for the argument: raw=%s (%s)" % (raw, type(raw)) )
        ...
        if txt:
            ...
            if raw: typeMsg = 'raw'
            try:
                TLX.instance().log_testcase_error(message=txt,component=TC, tcid = self.__id, bold=bold,
                        italic=italic, multiline=multiline, typeMsg=typeMsg, fromlevel=LEVEL_TE, 
                        tolevel=LEVEL_USER, testInfo=self.__testInfo)
            except UnicodeEncodeError:
                TLX.instance().log_testcase_error(message=txt.encode('utf8'),component=TC, tcid = self.__id, bold=bold,
                        italic=italic, multiline=multiline, typeMsg=typeMsg, fromlevel=LEVEL_TE, 
                        tolevel=LEVEL_USER, testInfo=self.__testInfo)
    ...
    def warning (self, txt, bold = False, italic=False, multiline=False, raw=False):
        ...
        if not isinstance(bold, bool): raise Exception("tc>warning: bad value for the argument: bold=%s (%s)" % (bold, type(bold)) )
        if not isinstance(italic, bool): raise Exception("tc>warning: bad value for the argument: italic=%s (%s)" % (italic, type(italic)) )
        if not isinstance(multiline, bool): raise Exception("tc>warning: bad value for the argument: multiline=%s (%s)" % (multiline, type(multiline)) )
        if not isinstance(raw, bool): raise Exception("tc>warning: bad value for the argument: raw=%s (%s)" % (raw, type(raw)) )
        ...
        if txt:
            ...
            try:
                TLX.instance().log_testcase_warning(message=txt,component=TC, tcid = self.__id,
                        bold=bold, italic=italic, multiline=multiline, typeMsg=typeMsg, fromlevel=LEVEL_TE, 
                        tolevel=LEVEL_USER, testInfo=self.__testInfo)
            except UnicodeEncodeError:
                TLX.instance().log_testcase_warning(message=txt.encode('utf8'),component=TC, tcid = self.__id,
                        bold=bold, italic=italic, multiline=multiline, typeMsg=typeMsg, fromlevel=LEVEL_TE, 
                        tolevel=LEVEL_USER, testInfo=self.__testInfo)
    ...
    def pause(self, testId, stepId, stepSummary, timeout=86400.0):
        ...
        cmd_data = { 'cmd': Messages.CMD_INTERACT, 'pause': True, 
                        'step-id':  str(stepId), 'step-summary': stepSummary, 
                        'test-id': testId, 'timeout': timeout } 
        cmd_data.update( TLX.instance().getTestInfo() )
    ...
    def interact(self, ask, timeout=30.0, default=None):
        ...
        if default is not None: cmd_data.update({'default': default})
        cmd_data.update( TLX.instance().getTestInfo() )
    ...
    def sendNotifyToAgent(self, adapterId, agentName, agentData):
        ...
        if not _ExecuteTest:
            return
        cmd_data = {    'event': "agent-notify", 'destination-agent': agentName,
                        'source-adapter': str(adapterId), 'data': agentData, 
    ...
    def sendReadyToAgent(self, adapterId, agentName, agentData):
        ...
        if not _ExecuteTest:
            return
        cmd_data = { 'event': "agent-ready", 'destination-agent': agentName,
                    'source-adapter': str(adapterId), 'data': agentData } 
        cmd_data.update( TLX.instance().getTestInfo() )
        TCI.instance().notify(data=cmd_data)
    ...
    def sendInitToAgent(self, adapterId, agentName, agentData):
        ...
        if not _ExecuteTest:
            return
        cmd_data = { 'event': "agent-init", 'destination-agent': agentName,
                    'source-adapter': str(adapterId), 'data': agentData } 
        cmd_data.update( TLX.instance().getTestInfo() )
        TCI.instance().notify(data=cmd_data)
    ...
    def sendResetToAgent(self, adapterId, agentName, agentData):
        ...
        if not _ExecuteTest:
            return
        cmd_data = { 'event': "agent-reset", 'destination-agent': agentName, 
                    'source-adapter': str(adapterId), 'data': agentData } 
        cmd_data.update( TLX.instance().getTestInfo() )
        TCI.instance().notify(data=cmd_data)
    ...
    def sendAliveToAgent(self, adapterId, agentName, agentData):
        ...
        if not _ExecuteTest:
            return
        cmd_data = { 'event': "agent-alive", 'destination-agent': agentName, 
                    'source-adapter': str(adapterId), 'data': agentData } 
        cmd_data.update( TLX.instance().getTestInfo() )
        TCI.instance().notify(data=cmd_data)
    ...
    def onRequest(self, client, tid, request):
        ...
        try:
            ...
            if request['cmd'] == Messages.RSQ_NOTIFY:
                ...
                #if __body__['script_id'] == TLX.instance().scriptId:
        ...
        # except AgentDownException as e:
            # self.abort(err="agent is down")
        except Exception as e:
            self.error("request error: %s" % e)
            
    def __getSubResult(self, inp, cond, out):
        """
        Sub funtion to get result condition
        internal function
        """
        result = False

        if cond == TestOperatorsLib.OP_EQUALS:
            if inp == out:
                result = True

        if cond == TestOperatorsLib.OP_NOT_EQUALS:
            if inp != out:
                result = True

        # new in v12
        if cond == TestOperatorsLib.OP_GREATER_EQUAL_THAN:
            GreaterThan = TestOperatorsLib.GreaterThan(x=out, equal=True)
            result = GreaterThan.comp(y=inp)

        if cond == TestOperatorsLib.OP_LOWER_EQUAL_THAN:
            LowerThan = TestOperatorsLib.LowerThan(x=out, equal=True)
            result = LowerThan.comp(y=inp)
        # end of new
        
        if cond == TestOperatorsLib.OP_GREATER_THAN:
            GreaterThan = TestOperatorsLib.GreaterThan(x=out, equal=False)
            result = GreaterThan.comp(y=inp)

        if cond == TestOperatorsLib.OP_LOWER_THAN:
            LowerThan = TestOperatorsLib.LowerThan(x=out, equal=False)
            result = LowerThan.comp(y=inp)

        if cond == TestOperatorsLib.OP_CONTAINS:
            Contains = TestOperatorsLib.Contains(needle=out, AND=True, OR=False)
            result = Contains.seekIn(haystack=inp)

        if cond == TestOperatorsLib.OP_NOT_CONTAINS:
            NotContains = TestOperatorsLib.NotContains(needle=out, AND=True, OR=False)
            result = NotContains.seekIn(haystack=inp)

        if cond == TestOperatorsLib.OP_STARTSWITH:
            Startswith = TestOperatorsLib.Startswith(needle=out)
            result = Startswith.TestOperatorsLib(haystack=inp)

        if cond == TestOperatorsLib.OP_NOT_STARTSWITH:
            NotStartswith = TestOperatorsLib.NotStartswith(needle=out)
            result = NotStartswith.seekIn(haystack=inp)

        if cond == TestOperatorsLib.OP_ENDSWITH:
            Endswith = TestOperatorsLib.Endswith(needle=out)
            result = Endswith.seekIn(haystack=inp)

        if cond == TestOperatorsLib.OP_NOT_ENDSWITH:
            NotEndswith = TestOperatorsLib.NotEndswith(needle=out)
            result = NotEndswith.seekIn(haystack=inp)
        return result
        
    @doc_public
    def condition(self, leftA, conditionA, rightA=None, 
                        leftB='', conditionB=None, rightB=None,
        ...
        resultA = self.__getSubResult(inp=leftA, cond=conditionA, out=rightA)
        if conditionB is not None:
            resultB = self.__getSubResult(inp=leftB, cond=conditionB, out=rightB)

        if conditionB is None:
            return resultA
        else:
            if AND:
