...
class TestTemplatesException(Exception): pass
...
class Template(object):
    ...
    def __init__(self, parent=None):
        ...
        if parent is not None:
            if not isinstance(parent, TestExecutorLib.TestCase):
                raise TestTemplatesException( 'ERR_ADP_011: testcase expected but a bad type is passed for the parent: %s' % type(parent) )
    @doc_public        
    def compare(self, template, expected):
        ...
        # compare templates
        ret, details = comparePayload(payload=msg.get(), tpl=tpl.get(), debug=False)
        
        if ret:
            if self.__testcase is not None:
                componentName = "TEMPLATE [matched]"
                TLX.instance().log_match_stopped( fromComponent = componentName, dataMsg = (msg.get(),details),
                                                    tcid = self.__testcase.getId(), font = 'italic', index=0,
                                                    fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testcase.getTestInfo() )
        else:
            if self.__testcase is not None:
                componentName = "TEMPLATE [mismatched]"
                TLX.instance().log_match_info( fromComponent = componentName, dataMsg = (msg.get(),details) ,
                                                tcid = self.__testcase.getId(), font = 'italic', index=0,
                                                fromlevel=LEVEL_TE, tolevel=LEVEL_USER, testInfo=self.__testcase.getTestInfo() )
    ...
    def getRaw(self, template):
        ...
        @rtype: string
        """
        if not isinstance(template, TemplateLayer) and not isinstance(template, TemplateMessage):
            raise TestTemplatesException('ERR_TPL_017: template layer message or layer expected, not %s' % type(template))
        return template.getRaw()
    @doc_public
    def getValue(self, template, layerName, layerKey, subLayerKey=None, caseSensitive=True):
        """
        ...
        if isinstance(template, TemplateLayer):
            if template.getName() == layerName:
                ...
                raise TestTemplatesException('ERR_TPL_018: template layer name unknown: %s' % layerName)
        ...
        if isinstance(template, TemplateMessage):
            ...
            if templateLayer is None:
                ...
                if templateLayer.getName() == layerName:
                    retTmp = templateLayer.get(key=layerKey, caseSensitive=caseSensitive)
                    if subLayerKey is not None:
                        ret = retTmp.get(key=subLayerKey, caseSensitive=caseSensitive)
                    else:
                        ret = retTmp
                else:
                    raise TestTemplatesException('ERR_TPL_018: template layer name unknown: %s' % layerName)
    ...
    def __decodeList(self, data, firstCall=False, name=''):
        ...
        for i in xrange(len(data)):
            ...
            if isinstance(data[i], dict):
                ...
                if len(data[i]) == 2:
                    layer_data2 = self.__decodeDict(data= { data[i][0]: data[i][1] } ) 
                else:
...
class TemplateLayer(object):
    ...
    def __init__(self, name):
        ...
        """
        if not ( isinstance(name, str) or isinstance(name, unicode) or isinstance(name, int) or isinstance(name, TestOperatorsLib.Any) 
                or isinstance(name, TestOperatorsLib.Startswith) or isinstance(name, TestOperatorsLib.NotStartswith) \
                or isinstance(name, TestOperatorsLib.Endswith) or isinstance(name, TestOperatorsLib.NotEndswith) or isinstance(name, TestOperatorsLib.Contains) \
                or isinstance(name, TestOperatorsLib.NotContains) or isinstance(name, TestOperatorsLib.GreaterThan) or isinstance(name, TestOperatorsLib.LowerThan) \
                or isinstance(name, TestOperatorsLib.RegEx) or isinstance(name, TestOperatorsLib.NotRegEx) \
                or isinstance(name, TestOperatorsLib.NotGreaterThan) or isinstance(name, TestOperatorsLib.NotLowerThan) ):
            raise TestTemplatesException('ERR_TPL_001: string expected on key name for the template layer: %s' % type(name))
    ...
    def addKey(self, name, data):
        ...
        if not ( isinstance(name, str) or isinstance(name, unicode) or isinstance(name, int) or isinstance(name, TestOperatorsLib.Any)
                ...
                or isinstance(name, TestOperatorsLib.RegEx) or isinstance(name, TestOperatorsLib.NotRegEx) \
                or isinstance(name, TestOperatorsLib.NotGreaterThan) or isinstance(name, TestOperatorsLib.NotLowerThan) ):
            raise TestTemplatesException('ERR_TPL_002 string expected on key name: %s' % type(name))

        if not ( isinstance(data, TemplateLayer) or  isinstance(data, int) or  isinstance(data, str) or  isinstance(data, unicode)
                or isinstance(data, TestOperatorsLib.Any) or isinstance(data, TestOperatorsLib.Startswith) or isinstance(data, TestOperatorsLib.NotStartswith) \
                or isinstance(data, TestOperatorsLib.Endswith) or isinstance(data, TestOperatorsLib.NotEndswith) or isinstance(data, TestOperatorsLib.Contains) \
                or isinstance(data, TestOperatorsLib.NotContains) or isinstance(data, TestOperatorsLib.GreaterThan) or isinstance(data, TestOperatorsLib.LowerThan) \
                or isinstance(data, TestOperatorsLib.RegEx) or isinstance(data, TestOperatorsLib.NotRegEx) \
                or isinstance(data, TestOperatorsLib.NotGreaterThan) or isinstance(data, TestOperatorsLib.NotLowerThan) ):
            raise TestTemplatesException('ERR_TPL_003 string or templatelayer expected on value: %s' % type(data))
...
class TemplateMessage(object):
        ...
        if name is not None:
            ...
            for l in self.__tpl__:
                if l.getName()==name:
                    data = l
                    if key is not None:
                        data = l.get(key=key, caseSensitive=keyCaseSensitive)
                    break
            return data
        else:
            rlt = []
    ...
    def addLayer(self, layer):
        ...
        if not isinstance(layer, TemplateLayer):
            raise TestTemplatesException('ERR_TPL_004: add layer assertion failed, templatelayer is need')
...
def comparePayload(payload, tpl, debug):
    ...
    # List expected, raise an expection if it is not the case, can happens only on unitary test
    if not isinstance( payload, list):
        TLX.instance().error( "ERR_TPL_006: compare: list expected but %s passed on template received" % type(payload) )
        payload = []
    if not isinstance( tpl, list):
        TLX.instance().error( "ERR_TPL_007: compare: list expected but %s passed on template expected" % type(tpl) )
        tpl = []
    ...
    for i in xrange(len(tpl)):
        # Somes checks before to start the comparaison; tuple expected with just (key,value)
        if not isinstance( tpl[i], tuple):
            TLX.instance().error( "ERR_TPL_008: compare: tuple expected but %s passed on template expected" % type(tpl[i]) )
            tpl[i] = ('',{})
        
        # Extract key,value from the payload and the template with index i
        tplKey, tplValue = tpl[i]
        
        # if the length of the payload if lower than the template than ignore the rest
        if i >= len(payload):
            match= False
            tpl_key, tp_dict = ignoreValues(tplValue=(tplKey, tplValue), color='$r')
        else:       
            # Somes checks before to start the comparaison; tuple expected with just (key,value)
            if not isinstance( payload[i], tuple):
                TLX.instance().error( "ERR_TPL_009: compare: tuple expected but %s passed on template received" % type(payload[i]) )
                payload[i] = ('',{})
            ...
            if not isinstance( tplValue, dict):
                TLX.instance().error( "ERR_TPL_010: compare: dict expected but %s passed on template expected" % type(tplValue) )
                tplValue = {}
            if not isinstance( plValue, dict):
                TLX.instance().error( "ERR_TPL_011: compare: dict expected but %s passed on template received" % type(plValue) )
                plValue = {}
            ...
            if not match:
                ...
                tpl_r, tpl_key = check_key_tuple(plKey=plKey, tplKey=tplKey, toStr=True)
                if not tpl_r: match = tpl_r

                # Reconstruct the rest and compute the final result of the comparaison
                tp_dict = {}    
                for d in tplValue.items():
                    ...
                    if not match:
                        ...
                        # check the dict key
                        if is_ops(k=k):
                            f, f2, t, i = check_all_ops_dict_key(pl=plValue, keyTpl=k, toStr=True) 
                            tmp_tp_key = t
                            pl_index = i
                            if not f:
                            ...
                            if match:
                                sub_match, sub_ret_tpl = check_dict_value(tpl_value=v, pl_value=plValue[pl_index], debug=debug)
                                if not sub_match:
                                    match = False
                            ...
                            tmp_tp_key = '$g%s' % k
                            # check the dict value
                            sub_match, sub_ret_tpl = check_dict_value(tpl_value=v, pl_value=plValue[k], debug=debug)
                            if not sub_match:
                                match = False
                            tmp_tp_value = sub_ret_tpl
                        else:
    ...
    for d2 in pl.items():
        ...
        try:
            ...
            try:
                ...
                TLX.instance().error( "ERR_TPL_012: template incorrect: dict key unknown: %s" % e )
                raise TestTemplatesException( "ERR_TPL_012: template incorrect: dict key unknown: %s" % e )
...
def check_all_ops_dict_value(pl, valueTpl, toStr=False):
    ...
    try:
        ...
        try:
            ...
            TLX.instance().error( "ERR_TPL_013: template incorrect: dict value unknown: %s" % e )
            raise TestTemplatesException( "ERR_TPL_013: template incorrect: dict value unknown: %s" % e )
...
def check_dict_value(tpl_value, pl_value, debug):
    ...
    if isinstance(tpl_value, tuple):
        recur_match, recur_tpl = comparePayload(payload=[pl_value], tpl=[tpl_value], debug=debug)
        sub_match = recur_match
        sub_ret_tpl =  recur_tpl[0]
    elif is_ops(k=tpl_value):
        f, f2, t = check_all_ops_dict_value(pl=pl_value, valueTpl=tpl_value, toStr=True) 
        sub_ret_tpl = t
        sub_match = f
        sub_match = f2
        if not f:
...
def is_ops(k):
    ...
    @type k:
    """
    if isinstance(k, TestOperatorsLib.Any) or isinstance(k, TestOperatorsLib.Startswith) or isinstance(k, TestOperatorsLib.NotStartswith) \
        or isinstance(k, TestOperatorsLib.Endswith) or isinstance(k, TestOperatorsLib.NotEndswith) or isinstance(k, TestOperatorsLib.Contains) \
            or isinstance(k, TestOperatorsLib.NotContains) or isinstance(k, TestOperatorsLib.GreaterThan) or isinstance(k, TestOperatorsLib.LowerThan) \
                or isinstance(k, TestOperatorsLib.NotGreaterThan) or isinstance(k, TestOperatorsLib.NotLowerThan) \
                    or isinstance(k, TestOperatorsLib.RegEx) or isinstance(k, TestOperatorsLib.NotRegEx):
        return True
    else:
        return False
...
def check_ops(pl, tp, clrOk='$g', clrKo='$r',  toStr=False):
    ...
    if isinstance( tp, TestOperatorsLib.Any): # cmp any
        ...
        # TLX.instance().error( "ERR_TPL_015: compare template: expected key type unknown: %s (%s)" % ( str(tp), type(tp)) )
...
# unit test
#pl = [
#           ( 'IP', { 'source-ip': '127.0.0.1', 'destination-ip': '127.0.0.1'} ),
#           ( 'TCP', { 'source-port': '60246', 'destination-port': '80', 'tcp-event':'connected'} ),
#           ( 'HTTP', { 'request': ( 'GET /.../ HTTP 1.1', {'method': 'GET', 'version': 'HTTP 1.1' } ) } ),
#           ( 'APP', { 'raw': 'xxxxxxxxx' } )
#       ]
#
#tpl = [
#           ( TestOperatorsLib.Any(),    { 'source': '127.0.0.1', 'destination-ip': TestOperatorsLib.Startswith('127') } ),
#           ( TestOperatorsLib.NotContains('TCP2'),  {} ),
#           ( TestOperatorsLib.Endswith('P'), { 'request': ( 'GET /.../ HTTP 1.1', {} ) } )
#       ]
#
#tpl = [
#           ( TestOperatorsLib.Any(),    { TestOperatorsLib.NotContains('source-ip'): '127.0.0.1', TestOperatorsLib.Endswith('source-ip'): '127.0.0.1' } ),
#       ]
#
#tpl = [
...
#
#tpl = [
#           ( TestOperatorsLib.Any(),    { TestOperatorsLib.NotContains('s2urce'): TestOperatorsLib.Any(), TestOperatorsLib.Endswith('source-ip'): '127.0.0.1' } ),
#           ( TestOperatorsLib.Contains('TCP'), {'tcp-event': TestOperatorsLib.Startswith('conn'),  TestOperatorsLib.Contains('source'): '60246', 'destination-port': TestOperatorsLib.GreaterThan('70') } ),
#           ( 'HTTP', { TestOperatorsLib.Contains('request'): ( TestOperatorsLib.Endswith('HTTP 1.1'), {'version': TestOperatorsLib.Contains('1.1') } ) } ),
