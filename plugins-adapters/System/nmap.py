...
class Nmap(TestAdapter.Adapter):
	@doc_public	
	...
	def execute(self, cmd):
		...
		layer_nmap = TestTemplates.TemplateLayer('NMAP')
		layer_nmap.addKey(name='output', data=out)
		tpl_rsp.addLayer( layer_nmap )
		
		if self.logEventReceived:
			self.logRecvEvent( shortEvt = "nmap event", tplEvt = tpl_rsp ) # log event 		

	@doc_public	
	def hasReceivedEvent(self, expected, timeout=1.0):
		"""
		Wait to receive "nmap event" until the end of the timeout.
		...
		@return: nmap event or none otherwise
		...
		layer_nmap = TestTemplates.TemplateLayer('NMAP')
		layer_nmap.addKey(name='output', data=expected)
		tpl_expected.addLayer( layer_nmap )
