...
class Ncat(TestAdapter.Adapter):
	...
	def execute(self, cmd):
		...
		@param cmd: ncat argument
		@type cmd: string
		"""
		run = "%s %s" % (NCAT_BIN, cmd)
		self.debug(run)
		ps = subprocess.Popen(run, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
		out, err  = ps.communicate() 
		if out is None:
			self.error("unable to run ncat command: %s" % err)
		
		out = out.decode('latin-1').encode("utf-8") 
		
		tpl_rsp = TestTemplates.TemplateMessage()
		layer_ncat = TestTemplates.TemplateLayer('NCAT')
		layer_ncat.addKey(name='output', data=out)
		tpl_rsp.addLayer( layer_ncat )
	...
	def hasReceivedEvent(self, expected, timeout=1.0):
		...
		@return: ncat event or none otherwise
		...
		layer_ncat = TestTemplates.TemplateLayer('NCAT')
		layer_ncat.addKey(name='output', data=expected)
		tpl_expected.addLayer( layer_ncat )
