...
class ConsumerClient(TestAdapterLib.Adapter):
	...
	def receivedNotifyFromAgent(self, data):
		...
		if 'cmd' in data:
			if data['cmd'] == AGENT_INITIALIZED:
				...
				record = data['result']
				consumer_rec = {"Partition": record.partition , 
	...
	def receivedErrorFromAgent(self, data):
		...
		"""
		if "cmd" in data:
			if data['cmd'] in [ CONNECT, CLOSE,CONSUME,POLL,POSITION,
																	HIGHWATER,SEEK,SEEK_TO_BEGINNING,
																	SEEK_TO_END,PARTITIONS_FOR,END_OFFSETS,
																	PARTITIONS_FOR_TOPIC,OFFSETS_FOR_TIMES,
																	TOPICS,SUBSCRIBE,UNSUBSCRIBE,SUBSCRIPTION	]:
	...
	def isConsumed(self, timeout=2,record=None):
		...
		if record == None:
			record = {"Partition": TestOperatorsLib.Any() , 
											...
											"Key": TestOperatorsLib.Any(), 
											"Value": TestOperatorsLib.Any() ,
											"Timestamp": TestOperatorsLib.Any() ,
											"Timestamp_type": TestOperatorsLib.Any() ,
