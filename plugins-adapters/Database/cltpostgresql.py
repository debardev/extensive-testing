...
class PostgreSQL(TestAdapterLib.Adapter):
	...
	def __init__(self, parent, host='127.0.0.1', user='', password='', port=POSTGRESQL_PORT, name=None, 
												...
												verbose=True):
		"""
		Adapter to connect on PostgreSQL database
		...
		@param port: destination port (default=SutAdapters.Database.POSTGRESQL_PORT)
		...
		self.__checkConfig()
		self.debug("psycopg2 version: %s" % psycopg2.__version__)
	...
	def doQuery(self, query, timeout=1.0, dbName=''):
		...
		ret = self.isExecuted(timeout=timeout) 
		...
		if self.connPtr is None:
			...
			if self.cfg['agent-support']:
				...
				try:
					...
					# log response event
					tpl = templates.db( host=self.cfg['host'], port=self.cfg['port'], user=self.cfg['user'], password=self.cfg['password'], 
																		 more=templates.executed(status=cursor.statusmessage, nbChanged=str(cursor.rowcount))  )
					self.logRecvEvent( shortEvt = "executed", tplEvt = self.encapsule(db_event=tpl) )
